...
// q = 90%
static const unsigned char _YTable_[64]  = {
        3,   2,   2,   3,   5,   8,  10,  12,
        2,   2,   3,   4,   5,  12,  12,  11,
        3,   3,   3,   5,   8,  11,  14,  11,
        3,   3,   4,   6,  10,  17,  16,  12,
...
// q = 90%
static const unsigned char _UVTable_[64] = {
        3,   4,   5,   9,  20,  20,  20,  20,
...
{
    std::cout << "(II) DataSink :: Creating memory buffer" << std::endl;

    create_obuffer(width, height);

    std::cout << "(II) DataSink :: Writing header information" << std::endl;
...
//    int ofsG = comp > 1 ? 1 : 0, ofsB = comp > 1 ? 2 : 0;
    ...
    std::cout << "(II) DataSink :: Processing dataSet(s)" << std::endl;

    const int nbMB = (width/8) * (height/8);

    for(int i=0; i<nbMB; i+=1){
        int dataSet[64];

        // Les donnees sont de type int16_t alors que la fonction
        // attend des int, a defaut de modifier la fonction, on fait
    ...
    std::cout << "(II) DataSink :: Writing file footer" << std::endl;
    ...
    std::cout << "(II) DataSink :: Closing the file" << std::endl;
